...
`tplfuncs` is a collection of **128 useful functions** for link:https://golang.org[Go] link:https://pkg.go.dev/text/template[templates].
...
There is a total number of **128 functions** contained in this package.
...
_(a total of **22 functions**: <<lineOrErr>>, <<line>>, <<sortLines>>, <<head>>, <<skipHead>>, <<tail>>, <<skipTail>>, <<trimLines>>, <<trimAll>>, <<notMatch>>, <<match>>, <<withoutEmptyLines>>, <<withoutLineComments>>, <<wrapLines>>, <<indentSpaceLines>>, <<prefixLines>>, <<indentTabLines>>, <<getLines>>, <<asString>>, <<regexpReplaceLine>>, <<filterLinesPrefix>>, and <<withoutLinesPrefix>>)_
...
`filterLinesPrefix` returns only lines that have the given prefix

**Signature**: `filterLinesPrefix(prefix string, input string) string`

==== withoutLinesPrefix

`withoutLinesPrefix` removes all lines that have the given prefix

**Signature**: `withoutLinesPrefix(prefix string, input string) string`
